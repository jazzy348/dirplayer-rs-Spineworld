...
fn test_mus_read_logon_request() {
    ...
    let msg = reader
        .read_mus_message(Some(&mut cipher))
        .expect("Failed to read logon request");
...
fn test_mus_read_logon_response() {
    ...
    let msg = reader
        ...
        .expect("Failed to read logon response");
    assert_eq!(msg.subject, "Logon");
    assert_eq!(msg.recipients.len(), 1);
    ...
    assert_eq!(
        msg.content,
        StaticDatum::String("Kepler: Habbo Hotel shockwave emulator".to_string())
    );
}

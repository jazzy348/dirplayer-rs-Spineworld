...
browser_e2e_test!(test_03_pandapang_load, |player| async move {
    ...
    player
        .step_until(datum("_movie.frame").equals(StaticDatum::Int(175)))
        .timeout(10.0)
        .await?;
    ...
    player
        .step_until(datum("_movie.frame").equals(StaticDatum::Int(215)))
        .timeout(10.0)
        .await?;
    ...
    player
        .step_until(datum("_movie.frame").equals(StaticDatum::Int(50)))
        .timeout(10.0)
        .await?;
    // wait until the invader is touching the ground
    player
        .step_until(sprite().member("b06bound").visible(1.0))
        .timeout(30.0)
        .await?;

...
pub fn parse_glyph_3d(r: &mut W3dBlockReader) -> Result<W3dPrimitive, String> {
    ...
    let bevel = if r.remaining() >= 4 {
        r.read_f32().unwrap_or(0.0)
    } else {
        0.0
    };
    log(&format!(
        "  Glyph3D: \"{}\" depth={:.2} bevel={:.2} ({} bytes remaining)",
        name,
        depth,
        bevel,
        r.remaining()
...
pub fn extrude_alpha_mask_to_mesh(
    ...
    let idx =
        |x: u32, y: u32| -> usize { ((y as usize) * (width as usize) + (x as usize)) * 4 + 3 };
            ...
            for x in 0..width as i32 {
                ...
                if layer + 1 == slab_masks.len() || !solid(layer + 1, x, y) {
                    append_quad(
                        ...
                        [[u0, v_bottom], [u0, v_top], [u1, v_top], [u1, v_bottom]],
                        [0.0, 0.0, -1.0],
                    );
                }
                if !solid(layer, x, y - 1) {
                    append_quad(
                        ...
                        [[u0, z0_uv], [u1, z0_uv], [u1, z1_uv], [u0, z1_uv]],
                        [0.0, -1.0, 0.0],
                    );
                }
                if !solid(layer, x, y + 1) {
                    append_quad(
                        ...
                        [[u0, z0_uv], [u0, z1_uv], [u1, z1_uv], [u1, z0_uv]],
                        [0.0, 1.0, 0.0],
                    );
                }
...
fn flatten_glyph(glyph: &OutlineGlyph, scale: f32, x_offset: f32) -> Vec<Vec<[f32; 2]>> {

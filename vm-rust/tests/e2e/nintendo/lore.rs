...
browser_e2e_test!(test_nintendo_lore_load, |player| async move {
    ...
    player
        .step_until(datum("_movie.frame").equals(StaticDatum::Int(144)))
        .timeout(30.0)
        .await?;
    ...
    player
        .step_until(datum("_movie.frame").equals(StaticDatum::Int(676)))
        .timeout(30.0)
        .await?;
    ...
    player
        .step_until(datum("_movie.frame").equals(StaticDatum::Int(503)))
        .timeout(30.0)
        .await?;

    player
        .step_until(datum("sprite(5).member.text").equals(StaticDatum::String("CORRECT".into())))
        .timeout(10.0)
        .await?;
